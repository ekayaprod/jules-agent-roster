...
graph TD;
    ...
    subgraph Services
        ...
        JulesAPI[js/Services/JulesAPI/JulesAPI.js]
    end

    subgraph UI Components
        JulesTerminal[js/UI/JulesTerminal/JulesTerminal.js]
        AgentCard[js/UI/AgentCard/AgentCard.js]
        Toast[js/UI/Toast/ToastNotification.js]
        Clipboard[js/UI/Clipboard/clipboard-utils.js]
    ...
    RosterApp -->|Initializes| JulesTerminal;
    ...
    JulesTerminal -->|Delegates to| JulesAPI;
    ...
    class FusionLab,FusionCompiler,FusionIndex,FusionAnimation,AgentPicker,RarityEngine,PinnedManager features;
    class JulesTerminal,AgentCard,Toast,Clipboard,SearchCtrl,ExportCtrl ui;
    class FormatUtils,DownloadUtils,PerformanceUtils,MarkdownRenderer,StorageUtils,PromptParser,DomUtils utils;
    class AgentRepo,JulesAPI services;
...
- **Backend APIs (`JulesAPI`)**: The Jules Terminal connects securely to external backend services for launching agent sessions, strictly enforcing local API key management.
...
- **Client-Side Foundation**: Built purely to ensure portability, performance, and immediate GitHub Pages compatibility (it requires a lightweight Node.js build step via `npm run build:roster` to compile `agents.generated.json`).
- **Colocated Features**: We use `js/Features` to group domains (like `Fusion`) independently to avoid a monolithic structure!
- **Asynchronous Execution API**: Rather than waiting on blocking HTTP connections, `JulesTerminal.js` utilizes continuous polling intervals against `getActivities` to simulate real-time terminal progress.
...
To ensure reliable data flow from the static file system to the UI, the system enforces a strict parsing and validation strategy. The application relies on `safeJsonParse` wrappers that capture raw response errors and decorate them with human-readable context. Rather than failing silently, invalid payloads trigger explicit warnings, ensuring broken configurations (`agents.generated.json`) are easily diagnosed without requiring deep network inspection.

### ⚠️ Single Source of Truth

The `.md` files in the `prompts/` directories are always the absolute source of truth over other configuration files in the repository. The JSON configuration (`agents.generated.json`) is a compiled artifact generated by `build-roster.js`. You are never to question what the roster payload updates with during the build process. If `agents.generated.json` updates and alters an agent because of differences in the underlying markdown files, trust the script's output. Never manually alter or revert `agents.generated.json` against the true `.md` sources.
