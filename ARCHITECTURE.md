...
The `.md` files in the `prompts/` directories are always the absolute source of truth over other configuration files in the repository. The JSON configuration (`roster-payload.json`) is a compiled artifact generated by `build-roster.js`. You are never to question what the roster payload updates with during the build process. If `roster-payload.json` updates and alters an agent because of differences in the underlying markdown files, trust the script's output. Never manually alter or revert `roster-payload.json` against the true `.md` sources.
