...
window.julesAPI.configure("your_api_key_here");
...
const session = await window.julesAPI.createSession(
    "You are Architect...",
...
const activities = await window.julesAPI.getActivities(session.name);
```
...
The `JulesAPI` manages the network boundary with the Jules backend.
...
3. **Global Attachment:** The service is instantiated and bound directly to `window.julesAPI` to allow universal access across the decoupled vanilla JS UI components without relying on a centralized dependency injector.
