...
1. **Fetching:** It fetches `agents.generated.json` via the native `fetch` API, implementing an exponential backoff retry mechanism (`fetchWithRetry`) for network resilience.
