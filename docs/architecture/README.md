...
2. Add a `<script>` tag to `index.html` _before_ `RosterApp.js`.
...
We have a two-pronged testing strategy to make sure the foundation is solid while keeping it fast and fun.

- **Benchmarks (The Performance):** Test the core constraints and parsing logic by firing up the benchmark suite:

  ```bash
  npm test
  ```

  _(Yes, `npm test` runs `benchmark.js`!)_
