...
async function run() {
  ...
  try {
    ...
    agents.forEach((agent, index) => {
      if (hasIssues(agent.name)) {
        console.log(
          `[agents.json] Index ${index}: Name '${agent.name}' has whitespace issues.`
        );
        issuesFound = true;
      }
      if (hasIssues(agent.category)) {
        console.log(
          `[agents.json] Index ${index}: Category '${agent.category}' has whitespace issues.`
        );
        issuesFound = true;
      }
    });
  ...
  try {
    ...
    for (const [key, agent] of Object.entries(customAgents)) {
      ...
      if (hasIssues(agent.name)) {
        console.log(
          `[custom_agents.json] Key '${key}': Name '${agent.name}' has whitespace issues.`
        );
        issuesFound = true;
      }
      ...
      if (!files.includes(expectedFilename)) {
        console.log(
          `[custom_agents.json] Key '${key}': Expected file '${expectedFilename}' not found in prompts/fusions/.`
        );

        // Try to find close match
        const match = files.find(
          (f) => f.toLowerCase() === expectedFilename.toLowerCase()
        );
        if (match) {
    ...
    console.error(
      'Failed to read custom_agents.json or prompts/fusions directory',
      e
    );
  }
