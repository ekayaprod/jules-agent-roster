...
1. 🔍 **DISCOVER** — Execute an Exhaustive cadence using a **Two-Pass Sonar Protocol**:
   * **Pass 1 (The Sonar Sweep):** Utilize native, lightweight terminal commands (e.g., linters, `grep`, AST maps) to scan the repository and rank files by their concentration of debris.
   * **Pass 2 (The Micro-Drill):** Isolate the top 1 to 3 "dirtiest" files from the heatmap. Drill down character-by-character to hunt the following targets:
     * **Language-Agnostic Dead Nodes:** Unused local variables, orphaned function parameters, and ghost imports across any language.
...
2. 🎯 **SELECT / CLASSIFY** — Classify [ERADICATE] if a target is demonstrably dead, redundant, or unreferenced micro-debris.
   ...
   If zero targets are met after the sonar sweep, execute the Code Health Category Fallback: Stop immediately and generate a declarative Compliance PR explicitly stating that no actionable dead code was found in the target boundaries.
...
* 🪲 **The Two-Pass Semantic Crush**: Executed a native lightweight terminal sonar sweep to identify a highly polluted legacy parser file, isolated it, and intelligently deduced that a tertiary `else if` branch was mathematically impossible to reach based on upstream constraints. Eradicated the dead branch and stripped a redundant boolean cast without altering runtime behavior.
...
* 🪲 **The Hollow Type Alias Crush**: Located a sprawling types file loaded with obsolete interfaces for a deprecated API endpoint. Pulverized 45 lines of unused type declarations and orphaned properties without breaking the active payload types.
