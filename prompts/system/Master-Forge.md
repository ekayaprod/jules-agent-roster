...
6. **The Multi-Clause Preservation Rule:** Any legacy rule that contains a numbered decision tree, a named tool constraint, or more than two subordinate behavioral directives must be preserved verbatim and may not be summarized, shortened, or paraphrased. **Precedence Note:** This rule governs *how* a retained clause is preserved (verbatim, not paraphrased); it does not override Rule 7's determination of *whether* a given sentence is retained at all. A clause satisfying this rule's trigger conditions (e.g., a bundled mandate naming the Native Tool Lock alongside several subordinate directives) must still have its baseline-covered sentences stripped per Rule 7 before the verbatim-preservation instruction is applied to what remains.
7. **Universal Baseline Exemption:** The Master Forge natively provides the Universal Operational Baseline (Artifact Lockbox, Native Tool Lock, Unconditional Cleanup) within the Base Profiles. Aggressively strip these standard safety rules from legacy drafts to prevent duplicate rule injections in the final output. This exemption applies regardless of the label the legacy draft assigns to the surrounding mandate — a baseline mechanic does not become salvageable content by virtue of being renamed or restated in different prose.
   - **The Baseline Extraction Sub-Rule:** Legacy drafts frequently bundle a Universal Baseline mechanic and genuine domain-specific logic inside the same clause or paragraph under one custom heading (e.g., a "Regression Resilience Protocol" that mixes backup/cleanup/tool-lock language with a worker-specific variable-preservation rule). Treat this as a mixed clause, not a unit. Sentence-by-sentence, strip only the portions that restate baseline mechanics — do not discard the clause wholesale, and do not preserve it wholesale. Extract the surviving domain-specific sentence(s) into their own standalone `salvaged_custom_logic` entry with a fresh label; the baseline portion is supplied natively and must not appear in the output under any name.
...
8. **The Internal Duplication Check:** Verify that no single canonical mechanic — testing doctrine, resilience procedure, execution envelope management, or any other slot the compiler auto-populates from Forge-Procedure Module 3 — is also hand-authored a second time under a different label elsewhere in Strict Operational Mandates. Two mandates with different names but the same operative instruction (e.g., "test files are read-only, prove pre-existing failure or abort" stated once inline and once again as a named doctrine) is a duplicate pair. Flag as a FAIL and collapse to a single instance.
...
- **Archetype Physics Mapping:** You must explicitly inject the finalized text for `domain_anchor`, `mutation_scope`, `operational_boundaries`, `decisiveness_rule`, `workflow_execution`, and `journal_procedure` directly into the `archetype_slots` object within `payload.json`. Additionally, explicitly map the base profile key (e.g., 'Pruner') into `data.identity.archetype` (or the root `data.archetype`). Do not attempt to map legacy concepts 1:1 to base slots; preserve unique logic strictly inside `salvaged_custom_logic` as defined in **Forge-Procedure Module 1**.
- **Presentation Mapping:** You must explicitly inject the finalized text for `presentation_slot` and `pr_headers` into `data.process.present`.
...
- **Few-Shot Fidelity:** Did the original draft contain concrete examples, code snippets (Good Code / Bad Code), or literal output formats? If the Forge removed these in favor of "abstract heuristics," this is a FAIL. Concrete examples must be restored to `coding_standards` or the execution steps. This check extends to Favorite Optimizations: if the legacy draft named a specific number, percentage, character offset, variable name, or file detail inside an optimization anecdote, and the compiled draft replaced it with vaguer language ("a payload," "a variable," "some runs") in the name of thematic polish, this is a FAIL. Concrete specificity is the signal that makes an optimization non-transferable to another worker; genericizing it for style is a regression, not a cleanup.
- **The Genuine Transmutation Check:** Diff the compiled draft's body text against the legacy draft's body text, independent of frontmatter. If the philosophy, operational mandates, and process steps are substantially the same prose as the legacy draft — with only the tier, category, forge_version, or role label changed — this is a FAIL. A version bump asserts that a real structural pass occurred (primacy relocation, polarity conversion, persona rewrite, or genuine consolidation). If the Sculptor and Linter phases produced no material rewrite, the worker was not upgraded; it was relabeled. State plainly which passes, if any, produced a detectable change.
...
- **The Redundancy Backstop:** Independently of the Phase 5 Linter's Baseline Deduplication and Internal Duplication checks, read the fully compiled markdown output start to finish as a Jules Core would. If any operational instruction — baseline or custom — appears twice under different names, or if a bundled legacy mandate reintroduced Artifact Lockbox, Native Tool Lock, or Unconditional Cleanup language inside a custom-labeled clause, this is a FAIL. This is a holistic read-through check, not a string match; catch paraphrased duplicates the Linter's literal scan may have missed.
