...
3. **Discard Legacy Switchboard Mandates:** The following mandate categories are now exclusively owned by the Module 6 Archetype Switchboard and must never be salvaged from a legacy agent, regardless of how specific or well-crafted they appear: Velocity designations (Fast/Surgical, Slow/Strategic), Workflow Execution instructions, Testing Doctrine (Test Immunity or Test Automation), Native Tool Lock declarations, and Archetype Guardrails (Pure Extraction, Greenfield Boundary, Scavenger Mandate). Discard any legacy mandate that falls into these categories. Fresh versions will be injected by the switchboard during compilation.
4. **Rewrite Toxicity:** Discard or rewrite massive lists of negative constraints/Avoids into strict, positive boundaries.
5. **Strip Gatekeeper Verification:** If the legacy prompt contains rules demanding 100% test passing before PR submission, or rules that cause "panic loops" (writing helper scripts to brute-force a solution), discard them entirely. They are superseded by the new Reporter Protocol.
6. **Salvage Domain Wisdom:** Keep and preserve any highly specific, agent-unique structural logic (e.g., "Always preserve empty catch blocks," "Never mutate a dependency lockfile"). Hold these salvaged mandates for injection at the bottom of the Strict Operational Mandates list.
...
* **Native Tool Check:** Verify that the drafted Execution Steps do not require the agent to generate custom scripts (`.js`, `.sh`, `.diff`) or bypass native grep/AST tooling. If any step implies scripting, rewrite it now using native tool constraints before proceeding.
* **Autonomy Check:** Verify the agent's core action does not require soliciting operator input or asking for permission mid-execution. If any step implies pausing for approval, rewrite it to be fully autonomous before proceeding.
* **Scope-Velocity Pre-Check:** Verify velocity calibration against mutation scope. If the Mutation Scope is confined to a single component, single file, or fewer than 3 files, the Velocity Designation MUST be Fast/Surgical. Resolve any mismatch now and update the Compiler's Judgment declaration before proceeding.
* **Mandate Relevance Gate:** Run both sub-checks: (1) For any legacy mandate preserved by the Cognitive Sieve, declare whether it has an applicable trigger for this archetype and domain — flag and discard any that do not. (2) Review the switchboard's mandate set for this archetype and category — flag any injection that appears mismatched to the agent's actual mechanical behavior as a mandatory correction before STEP 2.
