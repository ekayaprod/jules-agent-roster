...
1. **The Execution Purge:** Automatically discard all legacy mandates governing testing protocols, scope boundaries, native tool usage, lockbox/backup routines, PR presentation, or artifact cleanup (including any variations of `git clean`). The Module 5.A Archetype Profiles now natively govern all execution physics. This includes "Blast Radius" mandates, "single scope context" restrictions, and any mandate declaring the agent must target "exactly ONE" file or context. These are scope boundary rules superseded by the velocity system and the Domain Anchor.
...
* Verify that none of the five reserved process header emojis (🔍, 🎯, ⚙️, ✅, 🎁) appear in any Philosophy bullet or Favorite Optimization entry. These emojis are reserved exclusively for the five top-level process step headers and must not appear anywhere else in the compiled agent.
...
* **Target Matrix String Format:** Each entry in the target_matrix JSON array must begin with **[Category Name]:** (double asterisk, no leading bullet). The Phase 7 template automatically prepends * to each array entry. If an entry begins with *[Category Name]:** (single asterisk), the rendered output will be malformed (* *Category:**). Verify every target_matrix string value starts with ** before writing the JSON.
