...
2. `prompts/system/Forge-Protocol.md` (The Trust & Safety Heuristics, Switchboard, and Module 7 Autorun Sequence)
...
* **Autonomous Mode:** If it is "AUTO", execute the following native command to locate legacy `.md` files that lack the modern V68.0 operational boundaries signature:
  `grep -rl "Strict Operational Mandates" prompts/ | xargs grep -L "The Operational Boundaries"`
...
* Strictly follow Module 7's two-step architecture: Use STEP 1 (The Cognitive Buffer) to perform all audits and Clay Protocol reflections natively, and use STEP 2 (The Compilation) to assemble the final variables into the master `<OUTPUT_TEMPLATE>`.

### STAGE 4: TERMINAL STATE & OUTPUT
**1. Native File Update:** Do NOT output the compiled markdown template into the chat. You must use your native file modification tools to completely overwrite the locked target file with the newly compiled V68.0 architecture text.
...
**PR TITLE:** `🛠️ Auto-Forge: Upgraded [Extracted Name] to V68.0 Architecture`
