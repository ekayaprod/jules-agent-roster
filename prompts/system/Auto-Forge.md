...
- **Decoupled Velocity Generation:** Inject `execution_mandate` (`data.process.execute.execution_mandate`), `discovery_velocity_rule` (`data.process.discover.discovery_velocity_rule`), `execution_posture` (`data.process.execute.execution_posture`), `reporter_procedure` (`data.process.verify.reporter_procedure`), and `testing_doctrine` (`data.process.verify.testing_doctrine`) based on throughput and verification layers (Forge-Procedure Module 3).
