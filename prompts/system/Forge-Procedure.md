...
Operational: Treat the repository as a strictly read-only filesystem. The SEARCH/REPLACE API is disabled for all source code files. If obfuscated files break the parser, initiate a Graceful Abort on that file. Unconditional Cleanup: Run git clean -fd -e .jules/ before PR or Abort to wipe data dumps. Read-Only Override: Write operations are confined strictly to your designated output files.
...
Count constraints are enforced natively by compile_json.js. Consult the compiler output on failure.
