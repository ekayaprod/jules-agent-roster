...
const zeroTargetExitInstruction = (data.process?.present?.requires_total_replacement_override || isCore)
    ...
    const map = {
        ...
        'FORGE_VERSION': data.identity?.forge_version || 'V' + CURRENT_FORGE_VERSION,
        ...
        'PRIMARY_RESPONSIBILITY': formatSlot(data.archetype_slots?.domain_anchor, "The Primary Responsibility"),
        'THE_SCOPE': formatSlot(data.archetype_slots?.mutation_scope, "The Scope"),
        'EXECUTION_RULE': finalExecutionRule,
        'RESILIENCE_PROCEDURE': formatSlot((data.archetype_slots?.operational_boundaries || '') + ' ' + (profileKey === 'Analyzer' ? ANALYZER_OPERATIONAL_BASELINE : UNIVERSAL_OPERATIONAL_BASELINE), "The Resilience Procedure"),
        'DOMAIN_MODIFIERS': formatList(data.domain_modifier_mandates),
        'AUTONOMOUS_SELECTION': formatSlot(data.archetype_slots?.decisiveness_rule, 'The Autonomous Selection'),
        'WORKFLOW_EXECUTION': formatSlot(data.archetype_slots?.workflow_execution, 'The Execution'),
        'VERIFICATION_PROCEDURE': formatSlot(data.process?.verify?.testing_doctrine || '', 'The Verification Procedure'),
        'SALVAGED_MANDATES': formatList(data.salvaged_mandates),
        ...
        'CROSS_VECTOR_GRANTS': formatList(data.cross_vector_grants),
        ...
        'JOURNAL_PROCEDURE': formatSlot(data.archetype_slots?.journal_procedure, 'The Journal Procedure'),
        ...
        'TARGET_MATRIX': formatTargetMatrix(data.process?.discover?.target_matrix, data.identity?.tier),
        'PRIORITY_LANGUAGE': data.process?.select_classify?.priority_language || 'arbitrarily',
        ...
        'EXECUTION_STEPS': formatExecutionSteps(data.process?.execute?.execution_steps),
