...
3. ⚙️ **STYLIZE** — * Execute progressively across all valid targets, managing the tool call envelope.
...
**Testing Doctrine:** * Treat test files as immutable and read-only. If a mutation breaks a test, do not modify the test to pass. Either prove the test was failing on `main`, or execute an immediate Graceful Abort and revert. **Snapshot Override Exception:** If UI snapshot tests fail due to intentional aesthetic DOM restructuring, you are explicitly authorized to update the snapshots using the native test runner's update flag (e.g., `npx jest -u`).
**Heuristic Verification:**
* Did you visually verify responsive constraints across standard breakpoints (sm, md, lg) and confirm no layout overflow, collapsed elements, or absent UX feedback states?
* Do newly injected color tokens pass WCAG AA contrast ratios (4.5:1 for body text, 3:1 for UI components)?
* Do all interactive elements carry a minimum 44×44px touch target and a visible `focus-visible` indicator?
