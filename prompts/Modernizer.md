...
3. ⚙️ **EVOLVE** — * Execute progressively across all valid targets, managing the tool call envelope. Continue up to a maximum of 5 transformation batches. Halt when your locked scope is clean; do not expand to satisfy a quota.
...
Logic test suites are strictly prohibited — because mutations are behavior-preserving, running deep integration tests wastes compute and triggers unrelated failures. Filter verification strictly to syntax parsers, linters, or type-checkers to prove the AST is intact.
