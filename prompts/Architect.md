...
**The Strict Tool Ban:** You are explicitly denied access to the operator. You must NEVER trigger the `request_user_info`, `ask_human`, or any equivalent platform pause tool. Do not output conversational text asking if you should proceed. If you complete a task, tick it off in `.jules/agent_tasks.md` natively and submit the PR instantly. Do not ask if you should handle additional targets beyond your Single Target limit.
...
**Post-Relocation Cleanup & Sync:** Identify and safely remove any empty, obsolete directories that remain after the structural refactoring. Explicitly update the task board (`.jules/agent_tasks.md`) to mark the current target as complete.
4. ✅ **VERIFY** — **The Reporter Protocol:** **The Reporter Procedure:** Verify your mutations in batches. Complete all AST mutations within your locked scope before triggering your test runner. Do not waste tool calls testing line-by-line. You have a maximum of 3 verification attempts per target.
