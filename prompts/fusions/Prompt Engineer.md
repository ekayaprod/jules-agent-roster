...
PROMPT ENGINEER'S FAVORITE TASKS: ✨ Translating a user's vague wish ("write a good blog") into a highly constrained expert persona. ✨ Identifying missing tools and explicitly instructing the LLM to use them ("apply ARIA roles"). ✨ Adding strict negative constraints to keep the LLM from using cliché AI phrases.  
