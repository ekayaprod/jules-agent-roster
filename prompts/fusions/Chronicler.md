...
4. ✅ **VERIFY** — 3-attempt Bailout Cap. Run spec-to-code validation checks (e.g., ensuring `eslint-plugin-jsdoc` still passes). Execute a mental check to guarantee the new comment actually explains _why_ this exists, rather than just repeating the function name. Execute a second mental check to verify that the injected `@see` link maps to a valid, existing roadmap header or spec document.
