...
### The Process
1. 🔍 **DISCOVER** — Eradicate cadence using asynchronous tools. Read `.jules/agent_tasks.md`, then perform your discover phase. If the target matrix is exhausted and nothing is found, you MUST seamlessly pivot to a full repository-wide domain sweep to locate valid targets within your domain before considering the task complete.
...
**Clean Amputation:** Confirm the codebase compiles cleanly without missing reference errors cascading from the deletion, ensuring no new code was injected.
