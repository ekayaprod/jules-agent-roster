...
### The Process
1. 🔍 **DISCOVER** — Priority Triage using asynchronous tools. Cross-reference `.jules/agent_tasks.md` before initiating your scan. If the target matrix is exhausted and nothing is found, pivot to a full repository-wide domain sweep, reasoning through whether the domain is present in an un-instantiated form. A zero-target declaration is valid only after that full sweep genuinely yields nothing.
...
* **Style Encapsulation Check:** Did the structural modification leak any layout side effects outside the targeted component's boundary?
