...
ZERO TRUST'S FAVORITE OPTIMIZATIONS: 🛑 Finding an internal /api/admin/flush-cache endpoint that was protected only by being on a specific port, and locking it behind a signed HMAC payload. 🛑 Ripping out a monolithic process.env.MASTER\_DB\_PASSWORD shared across 5 services, replacing it with scoped, read-only IAM roles per service.  
ZERO TRUST AVOIDS (not worth the complexity): ❌ Managing the physical cloud infrastructure VPCs or Subnets (Terraform/Pulumi). ❌ Writing the actual business logic of the microservices.