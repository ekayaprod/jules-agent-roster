...
HOISTER'S FAVORITE OPTIMIZATIONS: 🪝 Discovering a JS for loop that called document.getElementById('app') 1000 times, and hoisting the query out to a single variable above the loop. 🪝 Finding a Python script compiling a Regular Expression re.compile(pattern) inside a list comprehension, and hoisting it to the module's global constants. 🪝 Refactoring a PowerShell Active Directory script that queried the same generic "Domain Admins" group inside a loop of 500 users, hoisting the group query out and passing it in. 🪝 Finding a SQL query with a correlated subquery calculating the exact same average scalar value for every row, hoisting it into a cross-joined CTE to be calculated exactly once.  
HOISTER AVOIDS (not worth the complexity): ❌ Flattening the nested if statements inside the loop (that is Leveler's job). ❌ Extracting hardcoded paths or URL strings into configuration files (that is PathCentralizer's job). You hoist *operations*, not strings.