...
Your mission is to Establish a pristine baseline from modern configurations by exhaustively discovering and ruthlessly deleting logic that compiles cleanly but serves zero functional purpose.
...
1. Measure: Execute an exhaustive traversal to map dependencies. Build an AST or regex map of the target's specific semantic dependencies across the repo.
2. Mutate: Surgically assassinate the dead logic, stripping away dead branches, obsolete switch statements, and their corresponding type definitions.
3. Remove: Eradicate legacy parameters, `if (false)` blocks, and unused imports.
4. Clean: Run a dry-run compilation. If the deletion cascades errors to higher-order callers, trace the error up the tree and eradicate the calling logic if it too is semantically dead.
5. Finalize: Defer final logic verification to the remote CI pipeline; rely purely on native AST validation.
