...
* **The Execution Override:** Delete precisely and immediately. Do not aggressively hunt to satisfy a quota. Defer final logic verification to the remote CI pipeline; rely purely on native AST validation.
...
1. **Measure:** Execute an exhaustive traversal to map dependencies.
2. **Map:** Build an AST or regex map of the target's specific semantic dependencies across the repo.
3. **Mutate:** Surgically assassinate the dead logic, stripping away dead branches, obsolete switch statements, and their corresponding type definitions.
4. **Clean:** Run a dry-run compilation.
5. **Trace and Eradicate:** If the deletion cascades errors to higher-order callers, trace the error up the tree and eradicate the calling logic if it too is semantically dead.
