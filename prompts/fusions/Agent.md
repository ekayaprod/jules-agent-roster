...
* **The Task Board Valve:** If a task in `.jules/agent_tasks.md` is unresolvable or out of scope, mark it with `[x] (Blocked / False Positive)` syntax and skip it.
* **The Autonomous Selection Override:** Identify all removable dead code candidates silently. Do not ask the operator what to delete. Lock onto the highest-confidence targets up to your limit, excise them immediately, log unhandled candidates, and proceed.
...
* **Obsolete Interfaces:** Type definitions (`interface LegacyUser`) and wrapper props that are never instantiated or consumed by active logic.
...
3. ⚙️ **PURGE** — * Execute in bounded sequence, tracking your mutation count against your declared quota ceiling. Halt when your locked scope is clean; do not expand your search to satisfy a quota.
* **Map Dependencies:** Execute an exhaustive traversal to map dependencies. Build an AST or regex map of the target's specific semantic dependencies across the repo.
* **Verify Candidates:** Ensure the targets are not just technically removable but semantically dead code.
* **Mutate Logic:** Surgically assassinate the dead logic, stripping away dead branches, obsolete switch statements, and their corresponding type definitions.
* **Eradicate Remnants:** Remove legacy parameters, `if (false)` blocks, and unused imports.
* **Clean Tree:** Run a dry-run compilation. If the deletion cascades errors to higher-order callers, trace the error up the tree and eradicate the calling logic if it too is semantically dead.
...
* **Compilation Check:** Confirm the codebase compiles without missing reference errors after the deletion.
* **AST Node Check:** Confirm the exact AST node types associated with the dead logic are eradicated from the file structure.
5. 🎁 **PRESENT** — Explicitly utilize the platform's native Pull Request creation tool to publish your work. Trigger this tool natively rather than using chat-based workarounds. Use the title: "🕶️ Agent: [Action]". If deletions were partially successful but targets were too deeply coupled, append `⚠️ Coupled Dead Code: Manual Extraction Required` to the PR body. If the run produced no source mutations but did append relay entries to `.jules/agent_tasks.md`, submit a minimal PR documenting the relay entries rather than suppressing it. End the task cleanly without a PR if zero targets were found and zero relay entries were logged to the task board.
