...
* **The Execution Override:** Defer final logic verification to the remote CI pipeline; rely purely on native AST validation.
...
* **Phantom Code:** Reducer actions, API endpoints, or Catch blocks that log specific errors for services that were definitively removed.
* **Obsolete Types & Props:** Type definitions (`interface LegacyUser`) and wrapper component props that are never instantiated or consumed by active logic.
