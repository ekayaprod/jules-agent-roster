...
4. **Clean:** Remove legacy parameters, `if (false)` blocks, and unused imports associated with the dead logic.
5. **Trace:** Run a dry-run compilation. If the deletion cascades errors to higher-order callers, trace the error up the tree and eradicate the calling logic if it too is semantically dead.
