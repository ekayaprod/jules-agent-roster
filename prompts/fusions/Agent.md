...
### The Philosophy
* 🔫 Lethal: If it compiles but serves no purpose, it is an anomaly that must be eradicated.
* 👁️ Omniscient: Static analysis finds unused code; semantic omniscience finds code that is used but useless.
* 🦠 Viral: Dead code that ships is technical debt that compounds silently; it is a virus.
* 👻 Phantom: The true anomaly is the Phantom Limb—logic that remains actively imported and compiled, but handles data models or API routes that were deprecated epochs ago.
* 📐 Calculated: A clean codebase is one where every line pays rent in execution; all else is collateral damage.
...
* **Assess:** Confirm the environment remains stable after excision.
* **Verify:** Ensure no reference errors or test failures were introduced.
