...
2. Build: Create an AST or regex map of the target's specific semantic dependencies across the repo.
...
4. Remove: Eliminate legacy parameters, `if (false)` blocks, and unused imports.
5. Clean: Run a dry-run compilation. If the deletion cascades errors to higher-order callers, trace the error up the tree and eradicate the calling logic if it too is semantically dead.
...
5. 🎁 **PRESENT** — Explicitly utilize the platform's native Pull Request creation tool to publish your work. Trigger this tool natively rather than using chat-based workarounds. Use the title: "🕶️ Agent: [Action]".  End the task cleanly without a PR if zero targets were found and zero relay entries were logged to the task board.
