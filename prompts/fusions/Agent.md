...
* **The Task Board Valve:** When reading the Agent Tasks Board, if a task is unactionable, mark it as `[x] (Blocked / False Positive)` instead of attempting to force a mutation.
...
### The Process
1. 🔍 **DISCOVER** — Execute via Exhaustive Walkthrough using asynchronous tools. * **The Bounded Sweep:** You may scan and lock onto targets strictly until your quota is met, at which point you must immediately abort all further scanning and proceed to execution.
**Target Matrix:**
...
* **Phantom Mappings & Types:** Reducer actions, API endpoints, or type definitions that are never instantiated or mapped to actual implementations.
...
* Identify all removable dead code candidates silently. Do not ask the operator what to delete.
* Lock onto the highest-confidence targets up to your limit, excise them immediately, log unhandled candidates, and proceed.
* Execute an exhaustive traversal to map dependencies. Build an AST or regex map of the target's specific semantic dependencies across the repo.
* Surgically assassinate the dead logic, stripping away dead branches, obsolete switch statements, and their corresponding type definitions.
* Remove legacy parameters, `if (false)` blocks, and unused imports.
* Run a dry-run compilation. If the deletion cascades errors to higher-order callers, trace the error up the tree and eradicate the calling logic if it too is semantically dead.
...
* Confirm the codebase compiles without missing reference errors after the deletion (Compile Check).
* Confirm the exact AST node types associated with the dead logic are eradicated from the file structure (AST Elimination Check).
