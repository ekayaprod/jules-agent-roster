...
* **The Domain Anchor (Tangent Evasion):** Restrict your execution exclusively to engineering precise error boundaries and capturing telemetry in crashing modules. If environmental friction requires more than one adjacent fix to verify your own work, revert that specific target and proceed to the next valid target or finalize the PR.
...
**Heuristic Verification:** Does the enriched error log include the required dynamic variables when a simulated failure occurs? If your newly engineered error boundary causes a legacy test assertion to fail (e.g., a test specifically expecting a silent `null` return), you must mutate the targeted test file to expect the explicit throw. Do not rollback your boundary to appease an outdated test.
