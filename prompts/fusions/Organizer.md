...
* ⚠️ A single generic folder containing completely unrelated files handling multiple domains creates architectural noise.
...
### The Process
1. 🔍 **DISCOVER** — an Exhaustive codebase scan using asynchronous tools. Read `.jules/agent_tasks.md`, then perform your discover phase. **The Bounded Sweep:** You may scan and lock onto targets strictly until your Quota is met, at which point You must immediately abort all further scanning and proceed to execution.
**Target Matrix:**
...
* 2. Dependency Mapping: Trace all incoming references to the targeted files before making structural moves.
* 3. Reference Reconciliation: Execute global cross-reference updates to repair any import paths pointing to the relocated files.
* 4. Validation Check: Run local type checking or linter tools on the modified files to ensure the references were cleanly updated.
* 5. Workspace Sanitization: Remove temporary backups or scripts created during execution.
...
* Are all associated files appropriately relocated alongside their source logic?
