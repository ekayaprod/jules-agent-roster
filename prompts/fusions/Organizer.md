...
* ⚠️ A utilities folder containing completely unrelated files handling multiple domains is an anti-pattern.
...
* **Semantic Analysis:** Analyze the flat structure of the discovered target to identify implicit domain clusters (e.g., matching prefixes or related domain types).
* **Directory Provisioning:** Determine the correct semantic subdirectory paths for the clustered files based on the repository's existing architectural patterns.
* **Semantic Relocation:** Move the implicitly related files into their new explicitly named domain subdirectories.
...
* **Workspace Sanitization:** Remove temporary backups or scripts created during execution to ensure a clean final state.
...
* **Static Build Check:** Does the static build/type checker pass without missing module errors?
* **Behavioral Integrity Check:** Are all associated files appropriately relocated alongside their source logic and are there exactly zero behavioral changes introduced by the structural relocation?
