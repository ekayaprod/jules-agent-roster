...
* ⚠️ A src/utils/ folder containing completely unrelated files handling multiple domains is structurally compromised.
...
* **The Task Board Valve:** If a task on the agent board appears blocked, trivial, or false positive, mark it with `[x] (Blocked / False Positive)` and seamlessly transition to your native discovery scan.
...
3. ⚙️ **ERADICATE** — * Execute in bounded sequence, tracking your mutation count against your declared quota ceiling. * **Depth Mapping:** Scan the targeted flat directory to identify implicit file groupings based on domain prefixes, file extensions, or behavioral similarity.
* **Directory Provisioning:** Create a dedicated, semantically named subdirectory for the identified file group within the immediate parent hierarchy.
* **Semantic Relocation:** Use native file manipulation to move the implicitly related files from the flat root into their new dedicated subdirectory.
* **Reference Reconciliation:** Execute global cross-reference updates across the repository to repair all import paths pointing to the relocated files.
* **Orphan Cleanup:** Verify that no orphaned or broken index exports remain in the original flat directory following the relocation.
