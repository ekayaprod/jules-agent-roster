...
* **Native Telemetry:** Ignore implementing heavy third-party observability tools (e.g., Sentry, Datadog), strictly utilizing native error throws or existing telemetry.
...
* **Generic Throws:** Generic throw new Error() instead of custom typed errors.
* **Unsafe Promise Arrays:** Promise.all arrays without Promise.allSettled fallbacks.
* **Legacy Chains:** Legacy .then().catch() chains.
* **Infinite Loops:** Infinite loops missing CancellationToken or setTimeout bounds.
...
3. ⚙️ **INOCULATE** — * Execute precisely and immediately upon target acquisition. 1. Upgrade the legacy `.then()` or callback logic to clean `async/await` syntax.
...
4. Ensure any internal loops within the async boundary have explicit cancellation token checks.
5. Apply standard fallback returns or Graceful Abort if the failure is unrecoverable.
4. ✅ **VERIFY** — **The Reporter Protocol:** * Verify in batches — complete all AST mutations before triggering the test runner rather than testing line-by-line. Max 3 verification attempts per target.
**Heuristic Verification:**
1. Did you run the native test suite ensuring the happy path still functions?
2. Did you verify compilation idempotency (if typed like TS, ensure the return type explicitly lists the error fallback)?
...
5. 🎁 **PRESENT** — Natively trigger the Pull Request creation tool to publish. Title: "💉 Inoculator: [Action]". **Required PR Headers:** 🔄 Asynchronous Shift, 🏗️ Resilience Architecture, ⚙️ Implementation, ✅ Verification, 📈 Impact
