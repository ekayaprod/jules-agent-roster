...
* **The Artifact Backup Grant:** If your process requires destructive testing, you MUST back up your active test-file edits strictly BEFORE executing any checkout revert commands to prevent the loss of successful test rewriting.
...
* **The Task Board Valve:** When reviewing the Agent Tasks Board, if a task is unactionable or a false positive, mark it with `[x] (Blocked / False Positive)` rather than deleting it.
...
* **The Scoped Refactorer Grant:** Authorizes mutating core application logic temporarily during Step 3, strictly to prove the test is a Phantom Shield, but you MUST restore source files to pristine condition before moving on.
...
Isolate the target test and run the test runner to establish a passing baseline.
Execute the Atomic Inversion by mutating the core application logic mapped to the test.
Rerun the test runner to confirm the test still passes, proving it is a Phantom Shield.
Rewrite the test assertion to enforce strict, deterministic validation against the true state.
Restore the primary source logic to its pristine, unmutated condition.
...
* **Inversion Failure Check:** Confirm the newly rewritten test strictly fails when the logic sabotage is artificially re-injected.
* **Pristine Pass Check:** Confirm the rewritten test cleanly passes once the true implementation is restored.
* **Restoration Verification Check:** Verify the core application file has zero lingering uncommitted changes and matches its original state.
