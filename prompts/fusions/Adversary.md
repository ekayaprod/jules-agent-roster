...
* **The Mutation Scope Constraint:** Constrained strictly to one paired test-and-source file combination at a time to prevent compounding AST errors.
* **The Execution Mandate Qualifier:** Map only the dependencies required to identify and execute a safe Atomic Inversion for the current target. Do not fall into open-ended repository cartography.
* **The Sandboxed Resilience Protocol:** Treat dependencies, lockfiles, and CI workflows as immutable read-only infrastructure. Adapt or execute a Graceful Abort if a tool fails 3 times.
...
* **The Revert Failsafe:** If your process requires destructive testing, you MUST back up your active test-file edits using the baseline lockbox procedure strictly BEFORE executing any `git checkout` revert commands to prevent the loss of successful test rewriting.
...
* Confirm the rewrite passes against the restored pristine logic.
...
* Confirm the assertions actually validate the core logic, not just mock objects or types.
