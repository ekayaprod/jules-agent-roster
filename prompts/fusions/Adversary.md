...
1. 🔍 **DISCOVER** — Execute Stop-on-First discovery. Mandate Sabotage Check to prove failure.
   ...
   * **Hunt for:** Identify exactly 5-7 literal anomalies (e.g., assertions solely reliant on `toBeDefined`, tests that mock the entire target module instead of external dependencies, shell scripts lacking `$? -eq 1` checks, missing schema shape validations, Python assertions using raw `is not None`).
