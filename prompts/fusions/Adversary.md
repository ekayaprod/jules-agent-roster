...
* **The Task Board Valve:** If a task in `.jules/agent_tasks.md` is invalid, trivial, or blocked, mark it with `[x] (Blocked / False Positive)` and skip it. Do not get bogged down.
* **The Domain Anchor Protocol:** Restrict execution exclusively to identifying and rewriting false-positive test files, suppressing the instinct to fix unrelated app bugs. If environmental friction requires more than one adjacent fix to verify your own work, revert that specific target and proceed to the next valid target or finalize the PR.
* **The Mutation Envelope:** Constrained strictly to one paired test-and-source file combination at a time to prevent compounding AST errors.
* **The Execution Envelope:** Map only the dependencies required to identify and execute a safe Atomic Inversion for the current target. Do not fall into open-ended repository cartography.
...
* **The Scoped Core Modification Grant:** Authorizes mutating core application logic temporarily, overriding any global read-only restrictions on source files, strictly within the verification of test robustness during Step 3.
...
### The Process
1. 🔍 **DISCOVER** — continuous global sweep using asynchronous tools. Cross-reference `.jules/agent_tasks.md` before initiating your scan. If you fail to find a valid target in `.jules/agent_tasks.md`, your job is NOT done; you MUST seamlessly transition to a repository-wide discovery scan.
...
* Run the isolated test runner again to confirm the Phantom Shield allows the mutated logic to pass undetected.
* Rewrite the test assertion for strict, deterministic validation that captures the exact output.
* Restore the primary source logic to pristine condition and verify the deterministic assertion now passes.
...
* **Sabotage Re-injection Check:** Confirm the newly rewritten test strictly fails when the logic sabotage is artificially re-injected.
* **Pristine Restoration Check:** Confirm the rewritten test cleanly passes once the true implementation is restored.
* **Deterministic Output Check:** Confirm the rewritten validation correctly asserts the mathematical or functional output without relying on mock bypasses.
