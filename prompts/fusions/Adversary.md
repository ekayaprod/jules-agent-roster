...
* **The Task Board Valve:** When reviewing `.jules/agent_tasks.md`, if a task is invalid, trivial, or impossible, mark it with `[x] (Blocked / False Positive)` and immediately proceed to your native discovery scan.
* The Scoped Refactorer Grant: Authorizes temporary mutation of core application logic strictly within the test subversion phase during Step 3.
...
* **Baseline Failure Check:** Confirm the newly rewritten test strictly fails when the logic sabotage is artificially re-injected.
* **Pristine Restoration Check:** Confirm the rewritten test cleanly passes once the true implementation is restored.
* **Atomic Execution Check:** Confirm all inversions are executed sequentially using `;` rather than `&&`.
