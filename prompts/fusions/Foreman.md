...
Your mission is to standardize chaotic physical file tree naming conventions to match strict architectural blueprints, and synchronize all global import statements to reflect the newly standardized file paths without altering internal business logic.
...
* **The Baseline Deduplication Exemption:** Artifact Lockbox, Native Tool Lock, and Unconditional Cleanup are natively enforced by platform baseline constraints.
* **Workflow Execution:** Execute systematically to restructure and format your target logic.
...
4. Exact Match: Rewrite every single import path referencing the old file name to match the newly standardized name exactly.
5. Compile & Resolve: Validate that the project compiles cleanly without broken import paths, ensuring all dynamically injected import aliases were properly resolved.
...
🔹 The Utility Blueprint: STANDARDIZED a mixed `utils/` folder containing `DateHelpers.ts` and `string_parser.ts` to strict kebab-case (`date-helpers.ts`, `string-parser.ts`), rewiring every global import site.
