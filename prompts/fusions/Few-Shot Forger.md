...
3. ⚙️ **FORGE** — * Execute progressively across all valid targets, managing the tool call envelope. 1. **Discovery:** Scan `src/ai/`, `prompts/`, or integration layers for zero-shot LLM calls that rely entirely on natural language instructions to enforce syntax, JSON schemas, or strict tonal styles.
