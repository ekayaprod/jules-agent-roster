...
**Target Matrix:**
* **CI/CD Secrets:** `.github/workflows/` or `.gitlab-ci.yml` missing AI environment secrets.
* **Docker Dependencies:** `Dockerfile` instances missing AI SDK dependencies or Python/Node runtime requirements.
* **Production Endpoints:** Hardcoded `localhost` endpoints for LLM services in production configs.
* **Vector DB:** Missing vector DB initialization scripts in infra-as-code manifests.
* **Model Versions:** Orphaned model version tags in `.env.example` templates.
