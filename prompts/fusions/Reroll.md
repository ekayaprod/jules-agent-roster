...
* Completely broken or deprecated function blocks causing silent runtime failures or chronically skipped tests in CI.
...
2. 🎯 **SELECT / CLASSIFY** — Silently classify targets using the Target Matrix. **Do not output a list of findings or pause to ask the operator for prioritization.** If multiple targets are found, arbitrarily lock onto the first valid targets up to your limit. Log any remaining unhandled targets into your `.jules/` journal, and immediately proceed to Step 3. Target Limit: 3 architectural rerolls per execution cycle.
