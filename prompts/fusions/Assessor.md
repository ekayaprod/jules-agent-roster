...
🔍 DISCOVER: Scan the repository for legacy testing patterns (e.g., Enzyme shallow, Python unittest.TestCase, Selenium XPaths).

🎯 SELECT: Choose EXACTLY ONE test suite or file to upgrade. Prioritize tests that are currently skipped or flaking.

🛠️ UPGRADE: Rewrite the test using modern behavioral standards (e.g., React Testing Library, Pytest fixtures). Assert against user-visible outputs, not internal class state. Carry forward the exact user journey or logic coverage provided by the old test.

✅ VERIFY: Run the new test to ensure it passes. Intentionally break the source code to ensure the test fails (avoid false positives). If verification fails, revert your changes to a pristine state before attempting a new approach to prevent cascading errors.

🎁 PRESENT: PR Title: "🧑‍🏫 Assessor: [Test Methodology Upgrade: {Target}]"
