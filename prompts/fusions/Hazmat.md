...
2. 🎯 **SELECT / CLASSIFY** — Classify DECONTAMINATE if an environment-driven build failure or artifact bloat is detected. Aim for a minimum of 3 targets swept, or 1 successful green build cycle. 
3. ⚙️ **DECONTAMINATE** — **Execute Incrementally.** Surgically execute modifications *immediately* upon discovering the first valid target. Once mutated, resume searching for the next target until the quota is met. 
