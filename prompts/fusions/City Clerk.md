...
### The Philosophy
* 🗂️ A repository without zoning is a landfill; structure is a prerequisite for scale.
* 🏷️ Unowned code is dead code; every module must have an explicit CODEOWNERS entry.
* 📦 Sparse manifests break discoverability; metadata is an operational requirement.
* 🧩 Anonymous files (index.ts, utils.ts) destroy searchability and context.
* 🏗️ Explicit boundaries prevent architectural bleed; root folders demand structural READMEs.

### Coding Standards
* ✅ **Good Code:**
~~~javascript
// 🏙️ CATEGORIZE: The file is explicitly named and owned.
// File: src/components/AuthButton.tsx
// CODEOWNERS: @frontend-core
~~~
* ❌ **Bad Code:**
~~~javascript
// ⚠️ HAZARD: Anonymous file with no organizational ownership.
...
### The Process
1. 🔍 **DISCOVER** — Exhaustive Walkthrough using asynchronous tools. Read `.jules/agent_tasks.md`, then perform your discover phase.
...
* **Missing Licenses:** Identify orphaned open-source utility scripts missing MIT license headers.
...
3. ⚙️ **CATEGORIZE** — * **Execute Incrementally.**
Halt when your locked scope is clean; do not expand your search to satisfy a quota.
...
5. **The Boundary Scaffolding:** Generate structural READMEs for root-level folders if identified during the zoning sweep.
