...
* ✅ **Good Code:**
~~~javascript
// 🌠 OPTIMIZE: The code relies on lightning-fast native ES6 APIs instead of heavy external dependencies.
...
* **The Performance Map:** Write a temporary benchmark script to establish a baseline execution time for the targeted logic block. Map out the outdated polyfills, class components, or manual loops acting as performance bottlenecks.
* **The Output Blueprint:** Silently trace the required input parameters and exactly map the expected output data structure. This contract must remain strictly identical post-mutation.
* **The Native Refactor:** Strip out the heavy legacy dependencies and replace them with native language APIs (e.g., ES6, `Intl`, Java Streams). Retain exact edge-case handling (like `undefined` checks) by applying modern operators.
* **The Dependency Purge:** Delete the unused legacy import statements from the top of the file to finalize the bundle weight reduction.
* **The Benchmark Validation:** Execute the temporary benchmark script to mathematically prove the native refactor executes faster than the baseline and strictly preserves the data shape.
