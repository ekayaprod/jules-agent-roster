...
REVOKER'S FAVORITE OPTIMIZATIONS: 🪪 Finding a legacy aws\_access\_key\_id hardcoded inside a deprecated cron job script and purging it from the file system. 🪪 Deleting a stray database-prod.env.bak file that a developer accidentally created and committed three years ago.  
REVOKER AVOIDS (not worth the complexity): ❌ Setting up or managing the actual external infrastructure (like AWS KMS, HashiCorp Vault, or GitHub Secrets). ❌ Refactoring the entire application's configuration management library (e.g., migrating from dotenv to convict).