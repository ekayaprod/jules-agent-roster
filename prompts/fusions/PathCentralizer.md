...
* **Decisiveness Rule:** Make firm decisions based on existing configuration mapping and static route paths.
* **Workflow Execution:** Execute refactoring logic only on route definitions.
...
3. ⚙️ **CENTRALIZE** — * Execute precisely and immediately upon target acquisition. Target Limit: 1 target.
    1. Identify scattered literal paths across the selected scope via an AST walkthrough.
    2. Extract the scattered literal paths and define them in a single configuration map (e.g., `config.ts`, `constants.py`, or `$Configuration` hashtables).
    3. Ensure any string interpolation logic remains intact during extraction.
    4. Replace the original literal strings with references to the new centralized map.
