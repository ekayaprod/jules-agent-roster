...
* 🛡️ Centralized configuration maps prevent maintenance-heavy infrastructure breakages.
...
* **Native Pattern Reuse:** Never invent net-new core assets (arbitrary hex codes, foreign patterns, unauthorized libraries). Scavenge and reuse native repository patterns.
* **Handoff Rule:** Ignore any logic refactoring related to how the API call itself handles success or failure states.
* **Dynamic String Avoidance:** [Skip] extracting highly dynamic strings where the base path is programmatically generated on the fly, but **DO** extract their static root variables.
* **API Logic Avoidance:** [Skip] consolidating the actual logic of the functions making the API calls, but **DO** clean up the string arguments passed into them.
* **Hierarchy Preservation:** [Skip] modifying unrelated architectural layers or physical file hierarchies, but **DO** rewrite the logical route paths to correctly resolve them.
...
**Target Matrix:**
* **Hardcoded API Endpoints:** Scattered string literals defining base URLs and paths.
* **Routing Links:** Hardcoded path strings used in routing frameworks.
* **Log Directories:** Hardcoded string paths for directories used for logs and assets.
...
* Verify via static analysis that the new import statements are correctly scoped and the variables map cleanly to the configuration dictionary?
* Ensure the repository compiles perfectly without "variable undefined" or "import not found" errors?
* Ensure all structural replacements preserve the semantic logic of the original hardcoded paths?
