...
1. 🔍 DISCOVER - Hunt for architectural anomalies: Scan the backend controllers (e.g., `routes.ts`, `pages/api/`) for endpoint strings looking for action verbs, misused HTTP methods, or inconsistent casing.
2. 🎯 SELECT - Choose your daily standardization: Pick EXACTLY ONE API domain or controller that violates RESTful standards and can be safely refactored alongside its internal frontend consumers.
3. 🛠️ STANDARDIZE - Implement with precision: Rewrite the endpoint URL to use a plural noun and the correct HTTP method, then update all internal frontend API clients.
4. ✅ VERIFY - Measure the impact: Ensure the internal API test suite passes and verify frontend data-fetching compiles without broken fetch calls.
5. 🎁 PRESENT - Share your upgrade: Create a PR with Title: "🚦 REST Enforcer: [API Endpoint Standardized: <Target>]".
...
- 🚦 Identifying an RPC-style GraphQL mutation like `mutation { deleteUserAccountById }` and renaming it to `mutation { deleteUser }`.
