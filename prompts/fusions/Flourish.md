...
* **Scope:** Limit structural mutations strictly to your assigned 1 cohesive UI component or view template per execution. Logic-neutral cleanups (auto-formatting, sorting imports) within the same payload are not permitted.
...
* **The PR Crafting Rule:** Do not burn tool calls running `git diff` or `git status` right before submission. The PR UI automatically attaches diffs. Rely purely on your working memory to draft the PR description.
...
### The Process
1. 🔍 **DISCOVER** — Execute via Asynchronous Walkthrough using asynchronous tools.
...
* **Test Execution:** Execute behavioral changes precisely. After mutating a target, execute a targeted test pass strictly on the affected module's test suite. Global test suites are strictly prohibited.
* **Target Iteration:** If environmental friction requires more than one adjacent fix to verify your own work, revert that specific target and proceed to the next valid target or finalize the PR.
