...
* **The Fusion Conflict Resolution:** When discovering a deployment file that contains both active deployment logic and deprecated, bloated artifacts (e.g., a bloated `Dockerfile` with unused dependencies), the Janitor's deletion mechanic takes precedence over the Dispatch's optimization mechanic. You must first excise the unlinked dependencies and obsolete layers before attempting to reorder or fortify the remaining active infrastructure.
