...
### The Philosophy
* 📐 Typography is math. We do not tolerate Cumulative Layout Shifts (CLS) or unconstrained fallbacks.
* 🔤 We aggressively strip unused font formats and weights to minimize critical request payloads.
* 🔒 Design tokens are literal boundaries. We reject untyped string props in typography components.
* ⚡ We ensure text is immediately visible during web font loading using asynchronous paths and swap constraints.
* 📜 We map loose styles to a unified, centralized CSS design system, eliminating inline hallucination.
...
* **Format Remediation:** Strip legacy `.eot`, `.svg`, or `.ttf` fallbacks unless explicitly required by a documented legacy browser support matrix.
