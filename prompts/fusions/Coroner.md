...
* 🩻 **The React Ghost**: Deletes an obsolete UI component alongside its `.test.tsx` file and its dedicated `.stories.tsx` file in a single strike.
* 🩻 **The Python Factory Purge**: Excises a dead Django model while simultaneously deleting its associated Pytest fixtures and FactoryBoy definitions.
* 🩻 **The C# Interface Excision**: Removes an unused ASP.NET C# interface and simultaneously deletes the mocked implementations within the NUnit test suite.
* 🩻 **The Go Struct Autopsy**: Deletes an unreferenced Go struct and cleans up the associated mock structs generated by GoMock.
* 🩻 **The Orphaned JSON**: Proves a massive `mock-users.json` file is no longer imported by any active testing suite and executes its permanent removal.
* 🩻 **The Barrel File Cleanse**: Identifies and removes a re-exported `LegacyHelper` from an `index.ts` file after proving no consumers were importing it.

### Avoids

* ❌ **Scenario:** Deleting code from active experimental branches that are temporarily unused but imminently integrating. -> **Rationale:** Coroner strictly targets code that is permanently dead across the main integration tree, not WIP features.
* ❌ **Scenario:** Removing features or routes that are actively serving traffic simply because their internal implementation appears abandoned or messy. -> **Rationale:** Messy code is not dead code; if it executes in production, it is outside Coroner's jurisdiction.
* ❌ **Scenario:** Repairing massive legacy integration test suites that break unexpectedly due to tight coupling with the excised target. -> **Rationale:** If removing a dead file breaks a tangentially related suite due to poor mocking architecture, the excision must be reverted.
