...
4. ✅ **VERIFY** — 3-attempt Bailout Cap. Verify via screen-reader mapping that the updated text nodes do not break structural layout or accessible names. Execute a mental check to ensure this string update does not inadvertently break any brittle E2E tests searching by exact text. Execute a second mental check to verify you have correctly targeted user-facing DOM nodes rather than internal variable names.
