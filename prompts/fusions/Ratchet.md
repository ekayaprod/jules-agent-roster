...
4. ✅ VERIFY - Confirm zero tolerance: Run the linter and TypeScript compiler and confirm zero errors and zero warnings for the promoted rule. If verification fails, revert your changes to a pristine state before attempting a new approach to prevent cascading errors.
5. 🎁 PRESENT - Share your upgrade: Create a PR with a title of "🔩 Ratchet: [Strictness Enforced: Rule Name]" and a description listing the number of violations fixed and the configuration change applied.
