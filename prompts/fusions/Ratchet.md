...
* Restrict execution strictly to modifying, optimizing, or parallelizing assigned execution logic. Limit mutations strictly to the targeted logic block. Logic-neutral cleanups (auto-formatting, sorting imports) within the same payload are not permitted.
...
The moment you identify ONE valid match from your Target Matrix, immediately abort all further scanning and proceed to execution. You must not compile a complete list of findings.
**Target Matrix:**
* **Implicit Anys:** Instances of `any` type definitions across legacy TypeScript files.
...
* **Stale Hooks:** React components missing exhaustive dependency arrays in `useEffect`.
...
3. ⚙️ **FIX** — Execute precisely and immediately upon target acquisition. Scope tunnel enforced: enter, execute, exit. Process exactly 1 target. Submit your PR the moment your single target is complete.
...
4. ✅ **VERIFY** — **The Reporter Protocol:** Complete all AST mutations within your locked scope before executing your heuristic checks. Do not waste tool calls verifying line-by-line. You have a maximum of 3 verification attempts per target.
...
5. 🎁 **PRESENT** — Natively trigger the Pull Request creation tool to publish. Title: "🔩 Ratchet: [Action]". If there are no warn level rules to upgrade, exit the pipeline.
**Required PR Headers:**
