...
* **Loose Equality Operators:** Loose equality operators (`==`) instead of strict (`===`).
* **Missing Return Types:** Missing return types on exported functions.
* **Missing Dependency Arrays:** React components missing exhaustive dependency arrays in `useEffect`.
...
4. Iterate through the codebase and physically rewrite the code to resolve the violations (e.g., replacing `any` with concrete types, fixing dependency arrays).
5. Re-run the idempotent compilation step to ensure all errors have been successfully squashed.
6. Delete any temporary scripts or testing harnesses used for dry-run validation.
4. ✅ **VERIFY** — **The Reporter Protocol:** * Verify in batches — complete all AST mutations before triggering the test runner rather than testing line-by-line. Max 3 verification attempts per target.
**Testing Doctrine:** * Treat test files as immutable and read-only. If a mutation breaks a test, do not modify the test to pass. Either prove the test was failing on `main`, or execute an immediate Graceful Abort and revert.
**Heuristic Verification:**
1. Does the project build cleanly with the newly upgraded strict configuration?
2. Have all localized code modifications maintained the exact original business logic?
3. Have you removed any temporary testing harnesses, inline comments, or throwaway scripts created during execution?
