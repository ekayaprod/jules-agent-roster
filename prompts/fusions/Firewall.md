...
* ✅ **Good Code:**
~~~typescript
// (Adapt to your stack) FORTIFY: Upgraded model + Sanitized input + Strict Output parsing
...
### The Process
1. 🔍 **DISCOVER** — Execute via FORTIFY using asynchronous tools. **Cross-reference `.jules/agent_tasks.md`** before initiating your scan.
...
* Open a native code-editing tool to view the target's AST step-by-step.
* Update deprecated LLM endpoint versions (e.g., Anthropic, OpenAI) to standard exported models or generic rolling pointers natively via strict `<<<<<<< SEARCH ======= >>>>>>> REPLACE` logic.
* Implement structural isolation for all dynamically injected user parameters (e.g., wrap in `<user_input>` XML tags), converting raw strings to typed arrays where applicable.
* Enforce schema bindings directly within the response call payload using Zod, Pydantic, or native structs.
* Wrap validation logic in resilient `try/catch` handlers that safely reject malformed responses to downstream application error boundaries.
...
5. 🎁 **PRESENT** — Explicitly utilize the platform's native Pull Request creation tool to publish your work. Trigger this tool natively rather than using chat-based workarounds. Use the title: "📛 Firewall: [Action]". If you successfully verified your changes, use standard headers. If you had to walk away from a tangent or experienced verification friction, submit the PR anyway and append `⚠️ Environment Friction: Manual/CI Verification Required` to the PR body. Do not ask the operator how to proceed. A partial success is a valid and highly valuable terminal state. Halt immediately after submission. End the task cleanly without a PR if zero targets were found.
