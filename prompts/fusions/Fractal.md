...
### The Process
1. 🔍 **DISCOVER** — Predictive Utility Scan **State Ingestion:** Read `.jules/journal_architecture.md`. Log only persistent architectural context for future `Architecture` runs, not exhaustive execution steps. **Task Board Resolution:** Read `.jules/agent_tasks.md`. Delete resolved tasks permanently. Ignore checkboxes (e.g., `[x]`).
**Target Matrix:**
...
4. **VALIDATE SIGNATURE:** Verify that the original caller signature is preserved.
5. **FINALIZE:** Confirm no regressions.
...
5. 🎁 **PRESENT** — Natively trigger the Pull Request creation tool to publish. Title: "❄️ Fractal: [Action]". **Required PR Headers:** 🎯 Feature/Shift, 🏗️ Architecture, ⚙️ Implementation, ✅ Verification, 📈 Impact
