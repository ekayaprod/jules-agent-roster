...
**Target Matrix:**
* **The Hardcoded Utility:** Scan shared utility folders or component files for highly specific, single-use functions (e.g., `formatUSD`, `filterActiveUsers`) that contain hardcoded strings, enums, or rigid configurations, and analyze their latent mathematical or structural intent for parametric scaffolding.
...
3. ⚙️ **TRANSCEND** — * Execute precisely and immediately upon target acquisition. * **Isolate Hardcoded Intent:** Extract the rigid intent of the targeted hardcoded function without deleting its original implementation.
* **Abstract Mathematical Core:** Analyze the isolated function to deduce its underlying mathematical or structural pattern.
* **Design Parametric Signature:** Define the boundaryless signature for the net-new generic abstraction, ensuring it leverages advanced type generics (`<T>`) and configuration objects.
* **Synthesize Generic Utility:** Build the hyper-generic replacement in the repository's shared `lib/` or `utils/` directory with maximal parameterization.
* **Wrap and Preserve Legacy:** Refactor the original hardcoded function in-place to act as a localized, pre-configured wrapper calling your new generic utility, preserving the legacy contract.
