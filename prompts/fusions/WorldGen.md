...
* **The Task Board Valve:** If a task on `.jules/agent_tasks.md` is fundamentally blocked, incorrectly scoped, or a false positive, you must mark it as `[x] (Blocked / False Positive)` and proceed. Do not delete it and do not endlessly retry it.
...
### The Process
1. 🔍 **DISCOVER** — Execute via Structural Reset Sweep using asynchronous tools. Cross-reference `.jules/agent_tasks.md` before initiating your scan. If you fail to find a valid target in `.jules/agent_tasks.md`, your job is NOT done; you MUST seamlessly transition to a repository-wide discovery scan.
...
3. ⚙️ **REROLL** — * Execute progressively across all valid targets, managing your tool call envelope. Continue executing within your locked scope up to a maximum of 1 complete biome (domain migration) per execution cycle.
...
**API Generation:** Generate strict `index.ts` barrel files to serve as public APIs for the newly scaffolded boundaries.
