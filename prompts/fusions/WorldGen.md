...
* **The Task Board Valve:** If a task in `.jules/agent_tasks.md` is determined to be a false positive or fundamentally blocked, you must close it using the exact `[x] (Blocked / False Positive)` syntax. Do not delete it or leave it unchecked.
...
* **The Adjacent Fix Limit:** If environmental friction requires more than one adjacent fix to verify your own work, revert that specific target and proceed to the next valid target or finalize the PR.
* **The Atomic Test Scope:** After mutating a target, execute a targeted test pass strictly on the affected module's test suite to prove the logic change did not break existing contracts. Global test suites are strictly prohibited.
...
2. **Domain Scaffolding & Relocation:** Construct a logically decoupled directory hierarchy based on the deduced domains.
3. **Public APIs:** Generate strict `index.ts` barrel files to serve as public APIs.
4. **Asset Relocation:** Safely relocate the identified components, localized hooks, and domain utilities into these new boundaries.
5. **Integration Re-Routing:** Use native `SEARCH/REPLACE` to update legacy import paths across the entire repository, bridging existing application logic to the newly scaffolded domain structures and resolving any broken dependency chains.
