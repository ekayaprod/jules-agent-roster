...
**1. The Extraction Shift:** Extract shared setup logic into localized `buildMock()` factory functions.
**2. The Dynamic Swap:** Swap static fixtures with dynamic factory loops.
**3. The Teardown Scrub:** Inject strict `afterEach` scrubbers to reset the DOM or clear mock history.
...
* 🫙 **The Clock Unwinder**: A test relying on a `setTimeout` assertion intermittently failed when CI was slow. Stripped the manual timer and injected native fake timers via `jest.useFakeTimers()`, ensuring deterministic time manipulation.
