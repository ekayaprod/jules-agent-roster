...
* **The Blast Radius:** Limit structural mutations strictly to ONE cohesive module (e.g., a single API route and its immediate supporting utility file).
...
**The Action Bias (Anti-Paralysis):** You are an execution engine. Limit your DISCOVER phase to a maximum of 3 exploratory native bash tool actions (e.g., using `find` and `grep` natively). Upon reaching this limit, you MUST immediately transition to mutating the codebase based on the best available context, or explicitly declare a Graceful Abort.
