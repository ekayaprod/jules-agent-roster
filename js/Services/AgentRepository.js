...
class AgentRepository {
  ...
  async fetchAgents() {
    try {
      const [rosterResponse, matrixResponse] = await Promise.all([
        ...
        NetworkUtils.fetchWithRetry('./fusion_matrix.json', { throwOn404: false }).catch(
          () => null,
        ),
      ]);
      ...
      const [payload, fusionMatrixData] = await Promise.all([
        ...
        matrixResponse && matrixResponse.ok
          ? this.safeJsonParse(matrixResponse, './fusion_matrix.json')
          : Promise.resolve({}),
      ]);
      ...
      return {
        ...
        fusionMatrix: this.fusionMatrix,
      };
    } catch (error) {
  ...
  validateAgentsData(agentPayload) {
    ...
    for (let i = 0; i < agentPayload.length; i++) {
      ...
      const isValid =
        agent && typeof agent.name === 'string' && validCategories.includes(normalizedCategory);
      ...
      if (agent.scope && typeof agent.scope !== 'string') {
        console.warn(`Sanitizing agent ${agent.name}: scope must be string. Casting.`);
  ...
  validateCustomAgent(key, agentPayload) {
    ...
    if (
      agentPayload.short_description !== undefined &&
      typeof agentPayload.short_description !== 'string'
    ) {
      console.warn(
        `[First Responder] Sanitizing ${key}: short_description must be string. Casting.`,
      );
    ...
    const maliciousPattern =
      /<\s*(script|iframe|object|embed|style|meta|link|base|svg|math|form|details|button|video|audio|canvas|map|area|plaintext|basefont|listing|xmp)\b|on\w+\s*=|javascript\s*:|vbscript\s*:/i;
    if (
