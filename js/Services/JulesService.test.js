...
describe('JulesService', () => {
  ...
  describe('_fetch', () => {
    it('should throw an error if API key is missing', async () => {
      await expect(service._fetch('test')).rejects.toThrow(
        'Jules API Key is missing. Please configure it in Settings.',
      );
    });
    ...
    it('should retry on 5xx server errors and eventually throw fallback message', async () => {
      ...
      await expect(service._fetch('test', {}, 2, 10)).rejects.toThrow(
        'We encountered a server error. Please wait a moment and try again.',
      );
      expect(global.fetch).toHaveBeenCalledTimes(3);
    });
  ...
  describe('getPullRequest', () => {
    it('should throw an error for unsupported source formats', async () => {
      await expect(service.getPullRequest('invalid-source', 1)).rejects.toThrow(
    ...
    it('should extract message from GitHub API error response', async () => {
      ...
      await expect(service.getPullRequest('sources/github/owner/repo', 1)).rejects.toThrow(
  ...
  describe('mergePullRequest', () => {
    it('should throw an error for unsupported source formats', async () => {
      await expect(service.mergePullRequest('invalid-source', 1)).rejects.toThrow(
        'Unsupported source format',
      );
    });
    ...
    it('should extract message from GitHub API error response', async () => {
      ...
      await expect(service.mergePullRequest('sources/github/owner/repo', 1)).rejects.toThrow(
        'Merge conflict',
      );
    });
  ...
  describe('closePullRequest', () => {
    it('should throw an error for unsupported source formats', async () => {
      await expect(service.closePullRequest('invalid-source', 1)).rejects.toThrow(
        'Unsupported source format',
      );
    });
    ...
    it('should extract message from GitHub API error response', async () => {
      ...
      await expect(service.closePullRequest('sources/github/owner/repo', 1)).rejects.toThrow(
        'Forbidden',
      );
    });
  ...
  describe('getPullRequests', () => {
    it('should throw an error for unsupported source formats', async () => {
      await expect(service.getPullRequests('invalid-source')).rejects.toThrow(
        'Unsupported source format for pull requests',
      );
    });
    ...
    it('should throw error on network error', async () => {
      global.fetch.mockRejectedValue(new Error('Network Error'));
      await expect(service.getPullRequests('sources/github/owner/repo')).rejects.toThrow(
        'Network Error',
      );
    });
