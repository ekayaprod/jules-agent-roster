...
class JulesManager {
  ...
  async loadSources() {
    ...
    const originalText =
      picker.options.length > 0 ? picker.options[0].textContent : '1. Select GitHub Repository...';
        ...
        if (!picker.dataset.listenerAttached) {
          picker.addEventListener('change', async (e) => {
            ...
            if (sourceName) {
              ...
              this.getEl('julesTerminal').innerHTML =
                `<div id="fetchingIndicator" class="term-session-line skeleton-pulse" style="color: var(--term-muted);">[SYS] Awaiting repository connection...</div>`;
            }
          });
