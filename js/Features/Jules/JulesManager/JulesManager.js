var getAgentUtils = () => (typeof window !== 'undefined' && window.AgentUtils) ? window.AgentUtils : ((typeof global !== 'undefined' && global.AgentUtils) ? global.AgentUtils : (typeof require !== 'undefined' ? require('../../../Utils/agent-utils.js') : null));
// Safe cross-environment getters
const getFormatUtils = () =>
  typeof window !== 'undefined' && window.FormatUtils
    ? window.FormatUtils
    : typeof global !== 'undefined' && global.FormatUtils
      ? global.FormatUtils
      : typeof require !== 'undefined'
        ? require('../../../Utils/format-utils.js')
        : null;
// Safe cross-environment getters
var getTelemetryUtils = () =>
...
const CORE_EMOJIS_REGEX = new RegExp(
  `(${Object.keys(CORE_EMOJIS)
    .map((k) => k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    .join('|')})`,
);
...
class JulesManager {
      ...
      try {
        ...
        if (window.julesService) {
          ...
          this.loadSources().catch((err) =>
            console.error('Background repository fetch failed', err),
          );
        }
      } finally {
        if (saveBtn)
  ...
  async loadSources() {
    ...
    const originalText =
      picker.options && picker.options.length > 0
        ? picker.options[0].textContent
        ...
        if (!picker.dataset.listenerAttached) {
          picker.addEventListener('change', async (e) => {
            ...
            if (sourceName) {
              ...
              this.getEl('julesTerminal').innerHTML = DOMUtils.getTerminalIndicatorHTML(
  ...
  async loadActiveSessionsForRepo(sourceName) {
    ...
    if (this.currentRepo !== sourceName) {
      ...
      if (!existingInd) {
        terminal.innerHTML += DOMUtils.getTerminalIndicatorHTML(
          'Checking active Jules routines...',
        );
      }
  ...
  async _fetchAndRenderSessions(sourceName, terminal) {
    ...
    for (let i = sessions.length - 1; i >= 0; i--) {
      ...
      const isEnded =
        ...
        s.state === 'ERROR' ||
        s.state === 'CANCELLED' ||
        (s.outputs && s.outputs.some(hasPullRequest));
  ...
  _checkEmptyTerminal() {
    ...
    if (
      ...
      (terminal.children.length === 0 ||
        (terminal.children.length === 1 && terminal.firstElementChild.id === 'fetchingIndicator'))
    ) {
      terminal.innerHTML = DOMUtils.getTerminalIndicatorHTML(
        'Ready. Awaiting execution commands...',
      );
    }
  }
  ...
  async launchSession(agent, btn = null) {
    ...
    const userTask =
      this.getEl('julesTaskInput').value.trim() ||
      'Analyze and optimize the repository based on your directives.';

    if (!sourceName) {
      this.app.toast.show(
        'Select a target repository first.',
        typeof TOAST_TYPES !== 'undefined' ? TOAST_TYPES.ERROR : 'error',
      );
      return;
    }

    if (btn) btn.disabled = true;
    await getAgentUtils().ensurePromptLoaded(agent, this.app.agentRepo);
    ...
    this.sessionQueue.push(async () => {
      try {
        await window.julesService.createSession(
          agent.prompt,
          userTask,
          sourceName,
          `${agent.name}`,
        ...
        this.app.toast.show(
          ...
          typeof TOAST_TYPES !== 'undefined' ? TOAST_TYPES.SUCCESS : 'success',
        );
        ...
        this.app.toast.show(
          `Could not launch the session: ${error.message || 'Unknown error'}`,
          typeof TOAST_TYPES !== 'undefined' ? TOAST_TYPES.ERROR : 'error',
          20000,
        );
