class TerminalPolling {
    ...
    this.manager.julesPollingIntervals[sessionId] = setInterval(async () => {
      try {
        ...
        if (this.manager.activeModalSessionId === sessionId) {
          ...
          if (contentEl) {
            contentEl.innerHTML = '';
            const pre = DOMUtils.createMarkdownPreBlock(
              fullHistoryMarkdown || 'No history available.',
            );
  ...
  _updatePollingState(sessionId, block, state, agentName, agentEmoji) {
    ...
    if (state.isCompleted) {
      ...
      clearInterval(this.manager.julesPollingIntervals[sessionId]);
      setTimeout(
        () => this.manager.dismissSession(sessionId),
        this.manager.constructor.SUCCESS_DISMISS_DELAY_MS,
      );
      return;
    }
    ...
    if (state.isWaitingForInput) {
      ...
      statusSpan.onclick = () => {
        this.manager.modals._showInteractionModal(
          sessionId,
          agentEmoji,
          agentName,
          state.rawMessage,
