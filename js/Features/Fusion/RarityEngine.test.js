...
describe('RarityEngine', () => {
  describe('getSuperDomain', () => {
    ...
    it('returns "Integrity" for testing, hygiene, security categories', () => {
      ...
      expect(RarityEngine.getSuperDomain({ category: 'security' })).toBe('Integrity');
    });
  ...
  describe('calculateRarity', () => {
    ...
    it('returns "Legendary" if one of the agents is Scavenger', () => {
      expect(RarityEngine.calculateRarity({ name: 'Scavenger' }, { name: 'Agent2' })).toBe(
        'Legendary',
      );
      expect(RarityEngine.calculateRarity({ name: 'Agent1' }, { name: 'Scavenger' })).toBe(
        'Legendary',
      );
    });

    it('returns "Legendary" for Micro/Macro Paradox: Architect + Pedant', () => {
      expect(RarityEngine.calculateRarity({ name: 'Architect' }, { name: 'Pedant' })).toBe(
        'Legendary',
      );
      expect(RarityEngine.calculateRarity({ name: 'Pedant' }, { name: 'Architect' })).toBe(
        'Legendary',
      );
    });
    ...
    it('returns "Epic" for QA Bridge: Integrity + Visible or Invisible', () => {
      expect(
        RarityEngine.calculateRarity(
          { name: 'A', category: 'testing' },
          { name: 'B', category: 'ux' },
        ),
      ).toBe('Epic');
      expect(
        RarityEngine.calculateRarity(
          { name: 'C', category: 'ux' },
          { name: 'D', category: 'hygiene' },
        ),
      ).toBe('Epic');
      expect(
        RarityEngine.calculateRarity(
          { name: 'E', category: 'testing' },
          { name: 'F', category: 'architecture' },
        ),
      ).toBe('Epic');
      expect(
        RarityEngine.calculateRarity(
          { name: 'G', category: 'operations' },
          { name: 'H', category: 'testing' },
        ),
      ).toBe('Epic');
    });
    ...
    it('returns "Rare" for Full-Stack Bridge: Visible + Invisible', () => {
      expect(
        RarityEngine.calculateRarity(
          { name: 'I', category: 'ux' },
          { name: 'J', category: 'architecture' },
        ),
      ).toBe('Rare');
      expect(
        RarityEngine.calculateRarity(
          { name: 'K', category: 'operations' },
          { name: 'L', category: 'docs' },
        ),
      ).toBe('Rare');
    });
    ...
    describe('Plus interactions', () => {
      ...
      it('returns "Common" for Plus Affinity: Plus + Matching Domain', () => {
        expect(
          RarityEngine.calculateRarity(
            { name: 'Bolt+', tier: 'Plus' },
            { name: 'A', category: 'architecture' },
          ),
        ).toBe('Common'); // Bolt+ matches Invisible
        expect(
          RarityEngine.calculateRarity(
            { name: 'B', category: 'architecture' },
            { name: 'Bolt+', tier: 'Plus' },
          ),
        ).toBe('Common');
        expect(
        ...
        ).toBe('Common'); // Palette+ matches Visible
        expect(
          RarityEngine.calculateRarity(
            { name: 'Sentinel+', tier: 'Plus' },
            { name: 'D', category: 'testing' },
          ),
        ).toBe('Common'); // Sentinel+ matches Integrity
      });
      ...
      it('returns "Uncommon" for Plus Bridge: Plus + Unmatched Domain', () => {
        ...
        ).toBe('Uncommon'); // Bolt+ (Invisible) + Visible
        expect(
          RarityEngine.calculateRarity(
            { name: 'F', category: 'testing' },
            { name: 'Bolt+', tier: 'Plus' },
          ),
        ).toBe('Uncommon'); // Integrity + Bolt+ (Invisible)
      });
    });
    ...
    it('returns "Uncommon" for Integrity Synergy: Integrity + Integrity', () => {
      expect(
        RarityEngine.calculateRarity(
          { name: 'A1', category: 'testing' },
          { name: 'A2', category: 'hygiene' },
        ),
      ).toBe('Uncommon');
    });
    ...
    it('returns "Common" for Backend Synergy: Invisible + Invisible', () => {
      expect(
        RarityEngine.calculateRarity(
          { name: 'A1', category: 'architecture' },
          { name: 'A2', category: 'operations' },
        ),
      ).toBe('Common');
    });
    ...
    it('returns "Common" as fallback safety for mixed unhandled domains', () => {
      expect(
        RarityEngine.calculateRarity(
          { name: 'Unknown1', category: 'fake' },
          { name: 'Unknown2', category: 'unknown' },
        ),
      ).toBe('Common');
      expect(
        RarityEngine.calculateRarity(
          { name: 'A', category: 'testing' },
          { name: 'B', category: 'fake' },
        ),
      ).toBe('Common');
    });
  });
  ...
  describe('getFusionDomain', () => {
    ...
    it('returns "12. Plus Glitch" or "13. Core Glitch" for identically named agents', () => {
      expect(
        RarityEngine.getFusionDomain(
          { name: 'Bolt+', tier: 'Plus' },
          { name: 'Bolt+', tier: 'Plus' },
        ),
      ).toBe('12. Plus Glitch');
      expect(RarityEngine.getFusionDomain({ name: 'Agent1' }, { name: 'Agent1' })).toBe(
        '13. Core Glitch',
      );
    });
    ...
    it('returns "10. Plus Paradox" or "11. Core Paradox" when Scavenger is combined', () => {
      // Plus Paradox
      expect(
        RarityEngine.getFusionDomain({ name: 'Scavenger' }, { name: 'Bolt+', tier: 'Plus' }),
      ).toBe('10. Plus Paradox');
      expect(
        RarityEngine.getFusionDomain({ name: 'Palette+', tier: 'Plus' }, { name: 'Scavenger' }),
      ).toBe('10. Plus Paradox');
      // Core Paradox
      expect(RarityEngine.getFusionDomain({ name: 'Scavenger' }, { name: 'Architect' })).toBe(
        '11. Core Paradox',
      );
      expect(RarityEngine.getFusionDomain({ name: 'Agent1' }, { name: 'Scavenger' })).toBe(
        '11. Core Paradox',
      );
    });

    it('returns "11. Core Paradox" for Architect + Pedant', () => {
      expect(RarityEngine.getFusionDomain({ name: 'Architect' }, { name: 'Pedant' })).toBe(
        '11. Core Paradox',
      );
      expect(RarityEngine.getFusionDomain({ name: 'Pedant' }, { name: 'Architect' })).toBe(
        '11. Core Paradox',
      );
    });
    ...
    it('returns "3. Domain Mastery" for same category', () => {
      expect(
        RarityEngine.getFusionDomain({ name: 'A', category: 'ux' }, { name: 'B', category: 'ux' }),
      ).toBe('3. Domain Mastery');
    });
    ...
    it('returns "5. Frontend Synergy" for Visible + Visible', () => {
      expect(
        RarityEngine.getFusionDomain(
          { name: 'A', category: 'ux' },
          { name: 'B', category: 'docs' },
        ),
      ).toBe('5. Frontend Synergy');
    });
    ...
    it('returns "7. Integrity Synergy" for Integrity + Integrity', () => {
      expect(
        RarityEngine.getFusionDomain(
          { name: 'A', category: 'testing' },
          { name: 'B', category: 'hygiene' },
        ),
      ).toBe('7. Integrity Synergy');
    });
    ...
    it('returns "6. Backend Synergy" for Invisible + Invisible', () => {
      expect(
        RarityEngine.getFusionDomain(
          { name: 'A', category: 'architecture' },
          { name: 'B', category: 'operations' },
        ),
      ).toBe('6. Backend Synergy');
    });

    it('returns "Unknown Domain" for fallback mixed domains', () => {
      expect(
        RarityEngine.getFusionDomain(
          { name: 'A', category: 'unknown_a' },
          { name: 'B', category: 'unknown_b' },
        ),
      ).toBe('Unknown Domain');
    });
  });
