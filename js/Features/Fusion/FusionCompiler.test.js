...
describe('FusionCompiler', () => {
  ...
  describe('fuse', () => {
    it('returns error if agent1 or agent2 is missing', () => {
      expect(compiler.fuse(null, {})).toEqual({
        name: 'Error',
        prompt: 'Invalid agents selected.',
      });
      expect(compiler.fuse({}, undefined)).toEqual({
        name: 'Error',
        prompt: 'Invalid agents selected.',
      });
    });
    ...
    it('returns a custom agent with fallback description', () => {
      ...
      const fused = c.fuse({ name: 'Architect', emoji: '📐' }, { name: 'Helix', emoji: '🧬' });
