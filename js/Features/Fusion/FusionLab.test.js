...
describe('FusionLab.getPreMergePreviewHTML Edge Cases', () => {
  ...
  test('should pass correct keys to fusionIndex.isUnlocked', () => {
    ...
    expect(fusionLab.fusionIndex.isUnlocked).toHaveBeenCalledWith(
      global.AgentUtils.getFusionKey('AgentA', 'AgentB'),
    );
...
describe('FusionLab Interaction Handlers and Edge Cases', () => {
  ...
  test('handleFusion displays specific error if fusion returns Error result', async () => {
    ...
    expect(fusionLab.showError).toHaveBeenCalledWith(
  ...
  test('renderFusionResult manages container state and appending custom agent card', () => {
    ...
    expect(mockElements.fusionResultContainer.classList.remove).toHaveBeenCalledWith(
      'fusion-revealed',
    );
  ...
  test('showResult configures UI for revealed state and focuses element', () => {
    ...
    expect(mockElements.fusionResultContainer.classList.add).toHaveBeenCalledWith(
      'fusion-revealed',
    );
...
describe('FusionLab Initialization and Bindings', () => {
  ...
  class MockFusionCompiler {
    constructor() {
      this.fusionMatrixMap = {};
      this.customAgentsMap = {};
      this.baseAgents = [];
    }
  }
  class MockFusionIndex {
    constructor() {
      this.init = jest.fn();
    }
  }
  class MockAgentPicker {
    constructor() {
      this.openPicker = jest.fn();
    }
  }
  class MockFusionAnimation {
    constructor() {}
  }
  ...
  test('bindEvents attaches click listeners correctly', async () => {
    ...
    const slotAHandler = fusionLab.elements.slotACard.addEventListener.mock.calls.find(
      (c) => c[0] === 'click',
    )[1];
    slotAHandler();
    ...
    const slotBHandler = fusionLab.elements.slotBCard.addEventListener.mock.calls.find(
      (c) => c[0] === 'click',
    )[1];
    slotBHandler();
    ...
    const fuseBtnHandler = fusionLab.elements.fuseBtn.addEventListener.mock.calls.find(
      (c) => c[0] === 'click',
    )[1];
    fuseBtnHandler();
    ...
    const resetBtnHandler = fusionLab.elements.resetLabBtn.addEventListener.mock.calls.find(
      (c) => c[0] === 'click',
    )[1];
    resetBtnHandler();
    ...
    const copyBtnHandler = fusionLab.elements.copyFusionBtn.addEventListener.mock.calls.find(
      (c) => c[0] === 'click',
    )[1];
