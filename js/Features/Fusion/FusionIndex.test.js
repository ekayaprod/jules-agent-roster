...
describe('FusionIndex', () => {
  ...
  it('loads state safely from storage utilities', () => {
    ...
    expect(mockStorageUtils.getJsonArrayItem).toHaveBeenCalledWith(
      'fusion_discovery_state',
      'FUSION_INDEX_PARSE_FAILED',
    );
  });
  ...
  it('renders the shelf and binds unlocked interactions', () => {
    ...
    const safeKeyA =
      typeof CSS !== 'undefined' && CSS.escape
        ? CSS.escape('AgentA,AgentB')
        : 'AgentA,AgentB'.replace(/(["\\])/g, '\\$1');
    ...
    const safeKeyC =
      typeof CSS !== 'undefined' && CSS.escape
        ? CSS.escape('AgentC,AgentD')
        : 'AgentC,AgentD'.replace(/(["\\])/g, '\\$1');
    ...
    expect(container.querySelector('.fusion-progress').textContent).toBe(
      '1 / 3 Protocols Discovered',
    );
  });
  ...
  it('executes without crashing when callback is undefined on interaction', () => {
    fusionIndex = new FusionIndex(
      'fusion-container',
      { 'AgentA,AgentB': { name: 'A' } },
      undefined,
    );
    mockStorageUtils.getJsonArrayItem.mockReturnValue(['AgentA,AgentB']);
    fusionIndex.init();
