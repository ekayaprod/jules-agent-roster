...
class AgentPicker {
    ...
    return itemsToRender.map((item) => {
      ...
      if (isCurrent) {
        renderedHtml = renderedHtml.replace(
          'class="mini-agent-card pop-in"',
          'class="mini-agent-card pop-in selected" aria-selected="true"',
        );
      }
  ...
  getChunkedHtml(htmlResults) {
    ...
    for (let i = 0; i < htmlResults.length; i += columns) {
      ...
      chunked.push(
        `<div style="display: grid; grid-template-columns: ${templateCols}; gap: 0.5rem; margin-bottom: 0.5rem;">${rowChunk}</div>`,
      );
    }
