...
const RarityEngine = (function () {
  ...
  function calculateRarity(agent1, agent2) {
    ...
    if (isScavenger1 || isScavenger2) {
      // Check Plus Paradox
      if (
        (isScavenger1 && ['Bolt+', 'Palette+', 'Sentinel+'].includes(agent2.name)) ||
    ...
    const isArchitectPedant =
      (agent1.name === 'Architect' && agent2.name === 'Pedant') ||
    ...
    if (
      domains.includes('Integrity') &&
      (domains.includes('Visible') || domains.includes('Invisible'))
    ) {
      return 'Epic';
    }
