 ...
 * @type {Object}
 * @see ../../../docs/architecture/Features/Search/SearchController.md#fuzzy-matching
 */
const FUSE_OPTIONS = {
 ...
 * @type {Object}
 * @see ../../../docs/architecture/Features/Search/SearchController.md#virtual-scrolling
 */
 ...
 * @see ../../../docs/architecture/Features/Search/SearchController.md#searchcontroller-architecture
 */
class SearchController {
   ...
   * @see ../../../docs/architecture/Features/Search/SearchController.md#initialization
   */
  constructor(app) {
    this.app = app;
  ...
  async filterAgents(query) {
    ...
    if (query.length > 0) {
      ...
      DOMUtils.setElementsDisplay(
        this.app.elements.grid || CONFIG.selectors.grid,
        'none',
        'searchResultsGrid',
      );
      DOMUtils.setElementsDisplay(
        this.app.elements.sectionHeader || CONFIG.selectors.sectionHeader,
        'none',
        'search-mode-header',
      );
    } else {
    ...
    let visibleCount = 0;
    const currentUnlockedSize =
      this.app.fusionLab && this.app.fusionLab.fusionIndex
        ? this.app.fusionLab.fusionIndex.unlockedKeys.size
        : 0;
    ...
    if (
      ...
      if (this.app.fusionLab && this.app.fusionLab.fusionIndex) {
        for (const key of this.app.fusionLab.fusionIndex.unlockedKeys) {
          let agent =
            AgentUtils.getCustomAgent(this.app.customAgents, key) ||
            this.app.fusionLab.compiler.customAgentsMap[key];
          if (agent) {
    ...
    const htmlResults = results.map((result) => {
      ...
      const renderedHtml =
        typeof cardHtml === 'string'
          ? cardHtml.replace(ANIMATION_DELAY_REGEX, `animation-delay: ${delay};`)
          : '';
      visibleCount++;
      return renderedHtml;
    });
    ...
    if (this.app.elements.announcer) {
      this.app.elements.announcer.textContent =
        results.length === 0 ? 'No protocols found.' : `Found ${results.length} protocols.`;
    }
  }
