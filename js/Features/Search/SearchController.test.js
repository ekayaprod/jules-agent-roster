...
global.Fuse = class {
  ...
  search(query) {
    return this.search.mock.results.length
      ? this.search.mock.results[this.search.mock.results.length - 1].value
      : [];
  }
};
...
const scriptFunc = new Function(
  'SearchControllerSrcCode',
  `eval(SearchControllerSrcCode + '; global.SearchController = SearchController;');`,
);
scriptFunc(searchControllerSrc);
...
describe('SearchController', () => {
  ...
  describe('Search Query Filtering', () => {
    it('should reset state and bail if query is empty', async () => {
      ...
      expect(DOMUtils.setElementsDisplay).toHaveBeenCalledWith(
        appMock.elements.grid,
        '',
        'searchResultsGrid',
      );
    });
  ...
  it('should catch exceptions thrown during Fuse.search', () => {
    global.Fuse = jest.fn().mockImplementation(() => ({
      search: jest.fn().mockImplementation(() => {
        throw new Error('Simulated Fuse Exception');
      }),
    }));
