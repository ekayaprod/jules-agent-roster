...
describe('ExportController', () => {
  ...
  describe('downloadCustomAgents', () => {
    it('downloads valid custom agents', () => {
      ...
      expect(global.FormatUtils.formatAgentPrompts).toHaveBeenCalledWith([
        appMock.customAgents['custom1'],
      ]);
      expect(global.DownloadUtils.downloadTextFile).toHaveBeenCalledWith(
        '# Custom Roster\nformatted agents',
        'jules_custom_agents.md',
      );
      expect(global.ClipboardUtils.animateButtonSuccess).toHaveBeenCalledWith(
        btnMock,
        'Downloaded!',
      );
    });
  ...
  describe('copyAll', () => {
    it('copies all core agents successfully', async () => {
      ...
      expect(global.FormatUtils.formatAgentPrompts).toHaveBeenCalledWith(appMock.agents);
      expect(global.ClipboardUtils.copyText).toHaveBeenCalledWith(
        '# Master Roster\nformatted core agents',
      );
      expect(appMock.toast.show).toHaveBeenCalledWith('Copied to clipboard');
      expect(global.ClipboardUtils.animateButtonSuccess).toHaveBeenCalledWith(btnMock, 'Copied!');
