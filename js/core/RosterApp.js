  ...
  renderAgents() {
    ...
    if (this.pinnedManager) {
      ...
      pinnedKeys.forEach((key) => {
        ...
        let agent =
          AgentUtils.getCustomAgent(this.customAgents, key) ||
          (this.fusionLab && this.fusionLab.compiler.customAgentsMap[key]);
        ...
        if (
          ...
          this.fusionLab.fusionIndex.isUnlocked(key)
        ) {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (nav && nav.classList.contains('search-active')) {
        if (
          ...
          (!this.elements.searchInput || this.elements.searchInput.value.trim() === '')
        ) {
          this.clearSearch();
        }
      }
        ...
        if (modal && contentArea && this.fusionLab && this.fusionLab.fusionIndex) {
          ...
          for (const key of potentialFusions) {
            ...
            if (isUnlocked) {
              const childAgent =
                AgentUtils.getCustomAgent(this.customAgents, key) ||
                this.fusionLab.compiler.customAgentsMap[key];
              if (childAgent) {
                const childIcon = FormatUtils.extractIcon(childAgent);
                const safeChildName = FormatUtils.escapeHTML(
                  FormatUtils.extractDisplayName(childAgent),
                );
                listItems += `
      ...
      if (
        actionBtn &&
        ['copy-agent', 'download-agent', 'launch-jules'].includes(actionBtn.dataset.action)
      ) {
        e.preventDefault();
        e.stopPropagation();
