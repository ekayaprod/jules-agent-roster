class EventBinder {
  static bind(app) {
    if (app.elements.searchInput) {
      const debouncedFilter = PerformanceUtils.debounce(
        (query) => app.filterAgents(query),
        SEARCH_DEBOUNCE_MS,
      );
    ...
    document.addEventListener('click', (e) => {
      ...
      if (nav && nav.classList.contains('search-active')) {
        if (
          ...
          (!app.elements.searchInput || app.elements.searchInput.value.trim() === '')
        ) {
          app.clearSearch();
        }
      }
      ...
      if (
        masterDropMenu &&
        masterDropMenu.classList.contains('visible') &&
        !masterDropMenu.contains(e.target) &&
        !masterDropBtn.contains(e.target)
      ) {
      ...
      app.activeDropdowns.forEach((menu) => {
        if (
          menu.id !== 'masterDropdownMenu' &&
          !menu.contains(e.target) &&
          !e.target.closest('[data-action="toggle-card-dropdown"]')
        ) {
      ...
      if (fusionsTarget) {
        ...
        for (const key of potentialFusions) {
          const isUnlocked =
            typeof unlockedKeys.has === 'function'
          ...
          const childAgent =
            AgentUtils.getCustomAgent(app.customAgents, fusionName) ||
            app.fusionLab.compiler.customAgentsMap[fusionName];
      ...
      if (pinTarget) {
        ...
        const existingPins = document.querySelectorAll(
          `[data-action="toggle-pin"][data-index="${safeIndex}"]`,
        );
      ...
      if (frontTarget && !e.target.closest('.fusion-quick-btn') && !e.target.closest('.pin-btn')) {
        ...
        if (agent.prompt === undefined) {
          ...
          promptArea.innerHTML =
            '<div style="display:flex; justify-content:center; align-items:center; height:100%;"><div class="loading-spinner" style="width:1.5rem; height:1.5rem; margin:0;"></div></div>';
      ...
      if (
        backTarget &&
        !e.target.closest('.prompt-scroll-area') &&
      ...
      if (
        actionBtn &&
        ['copy-agent', 'download-agent', 'launch-jules', 'download-parent-fusions'].includes(
      ...
      if (app.activeDropdowns && app.activeDropdowns.size > 0) {
        app.activeDropdowns.forEach((menu) => {
          ...
          const toggleBtn = document.querySelector(
            `[data-action="toggle-card-dropdown"][data-index="${toggleId}"]`,
          );
          if (toggleBtn) {
            toggleBtn.focus();
    ...
    if (app.elements.masterCopyBtn)
      app.elements.masterCopyBtn.addEventListener('click', (e) => app.copyAll(e.currentTarget));
    if (app.elements.masterDownloadCoreBtn)
      app.elements.masterDownloadCoreBtn.addEventListener('click', (e) => {
        app.downloadAll(e.currentTarget);
        if (masterDropMenu) masterDropMenu.classList.remove('visible');
      });
    if (app.elements.masterCopyFusionsBtn)
      app.elements.masterCopyFusionsBtn.addEventListener('click', async (e) => {
    ...
    if (app.elements.masterDownloadFusionsBtn)
      app.elements.masterDownloadFusionsBtn.addEventListener('click', (e) => {
