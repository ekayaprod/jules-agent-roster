...
class AgentCard {
  ...
  static getPromptNode(agent) {
    return DOMUtils.createMarkdownPreBlock(
      PromptParser.stripFrontmatter(agent.prompt || MESSAGES.NO_PROTOCOL_DATA),
    );
  }
  ...
  static create(agent, index, globalIndex) {
    ...
    if (agent.scope) {
      ...
      const safeScope = FormatUtils.escapeHTML(agent.scope);
      if (agent.scope.includes('Small')) scopeClass = 'scope-small';
      if (agent.scope.includes('Large')) scopeClass = 'scope-large';
      tags += `<span class="meta-tag ${scopeClass}">${safeScope}</span>`;
    }

    if (agent.tier) {
      const lowerTier = FormatUtils.escapeHTML(agent.tier.toLowerCase());
      const safeTier = FormatUtils.escapeHTML(agent.tier);
      tags += `<span class="meta-tag tier-badge tier-${lowerTier}">${safeTier}</span>`;
