...
class AgentCard {
    ...
    static create(agent, index, globalIndex) {
        ...
        const pinHtml = (index !== undefined && index !== null && index !== '' ) ? `<button class="icon-btn pin-btn ${pinClass}" data-action="toggle-pin" data-index="${index}" aria-label="Toggle Pin" aria-pressed="${String(Boolean(isPinned))}">📌</button>` : '';
