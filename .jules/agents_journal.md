...
## 2024-05-24 - 🦎 Chameleon - [Interactive States Mutated: Global Base Components]
**Learning:** Found multiple buttons and input elements across the codebase that were lacking proper `:focus-visible`, `:active`, and fully styled `:disabled` pseudo-class states resulting in flat, lifeless UI components that provide zero tactile feedback.
**Action:** Mutated all globally shared interactive elements (`button`, `input[type="text"]`, `a` variants) and component-specific controls (`.clear-search-btn`, `.icon-btn`, `.slot-card`, etc.) to include smooth `transition` properties alongside comprehensive `:hover`, `:focus-visible` (preferring outline rings to simple colors for accessibility), `:active` (transforming scale), and `:disabled` (opacity fading with `cursor: not-allowed`) states.
