## Pedant — Extracting Magic Numbers for Animation Delays
**Learning:** Found a hardcoded magic number `3500` for the burst delay duration scattered across `FusionAnimation.js` and its test suite. Hardcoded values cause stylistic entropy and prevent synchronization if the UI requirements change.
**Action:** Extracted `3500` into a strictly frozen, globally accessible constant `ANIMATION_DURATIONS.FUSION_BURST_MS` inside `js/constants/ui.js`, replaced all occurrences, and verified through the native Jest suite.

## Pedant — Extracting Magic Numbers
**Learning:** Hardcoded literal values like network timeouts (`10000`), backoff delays (`300`), retry limits (`3`), animation delays (`30`, `600`), loading overlay dismiss timings (`500`), and debounce delays (`300`), and modal focus delays (`50`) were scattered directly in method arguments and body implementations across files like `RosterApp.js` and `AgentPicker.js`. This creates stylistic entropy and prevents external modules (like test suites) from referencing the single source of truth.
**Action:** Always extract such literal boundaries into properly typed and scoped global constants (e.g., `REQUEST_TIMEOUT_MS`, `DEFAULT_BACKOFF_MS`, `MAX_RETRIES`, `ANIMATION_DELAY_STEP_MS`, `ANIMATION_DELAY_MAX_MS`, `LOADING_OVERLAY_DISMISS_MS`, `SEARCH_DEBOUNCE_MS`, `UI_TIMINGS.MODAL_FOCUS_DELAY_MS`) and export them if needed in testing to maintain absolute structural predictability.
